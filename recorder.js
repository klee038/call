...
const Recorder = {
  ...
  undo: function() {
    if (this.data.timeline.length > 0) {
      let lastAction = this.data.timeline.pop();
  ...
  redo: function() {
    if (this.redoStack.length > 0) {
      ...
      this.data.timeline.push(action);
    }
  },
  ...
  /**
   * PDF出力用：指定ゲームの「4行 × 60列」のマス目データを生成する
   */
  generateTableData: function(savedData, gameIdx) {
    let timeline = savedData ? savedData.timeline : this.data.timeline;
    let rowsCount = 4;
    
    // 60列(上段30マス＋下段30マス)の空配列を4行分つくる
    let table = Array.from({length: rowsCount}, () => new Array(60).fill(""));
    ...
    gameTimeline.forEach(action => {
      if (action.type === 'SR') {
        ...
        if (sR >= 0 && sR < rowsCount) table[sR][1] = "0";
        if (rR >= 0 && rR < rowsCount) table[rR][1] = "0";
        
      } else if (action.type === 'POINT') {
        let r = action.serverRow;
        if (r >= 0 && r < rowsCount && globalCol < 60) {
          table[r][globalCol] = action.score;
          globalCol++;
        }
      }
      // WIN_SCORE の特別な描画処理（マル表示など）は要望により削除しました
    });
