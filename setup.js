...
let defaultSettings = {
  ...
  hasSetting: true,
  hasCourtSelect: true, 
  isOfficialCall: false, // ★追加: 公式戦コールの初期値
  wakeLockMinutes: 3
};
...
let flowIsOfficialCall = false; // ★追加
...
function loadDefaultSettings() {
  ...
  flowIsOfficialCall = defaultSettings.isOfficialCall !== undefined ? defaultSettings.isOfficialCall : false; // ★追加
}
loadDefaultSettings();
...
function renderFlow() {
  ...
  if (flowStep === 1) {
    contentArea.innerHTML = `
      ...
      <!-- ★追加：コール設定トグル -->
      <div class="setting-row">
        <div class="setting-label">コール (第1G開始時)</div>
        <div class="toggle-group">
          <button class="toggle-item ${!flowIsOfficialCall ? 'selected' : ''}" onclick="setOfficialCall(false)">簡易版</button>
          <button class="toggle-item ${flowIsOfficialCall ? 'selected' : ''}" onclick="setOfficialCall(true)">公式戦</button>
        </div>
      </div>
...
function setOfficialCall(val) { flowIsOfficialCall = val; renderFlow(); } // ★追加
  ...
  content.innerHTML = `
    <div style="overflow-y: auto; max-height: 50vh; display: flex; flex-direction: column; gap: 5px; width: 100%; padding-right: 4px; box-sizing: border-box;">
      ...
      <div class="setting-row">
        <div class="setting-label">コール (第1G開始時)</div>
        <div class="toggle-group">
          <button class="toggle-item ${!tempSettings.isOfficialCall ? 'selected' : ''}" onclick="updateTempSetting('isOfficialCall', false)">簡易版</button>
          <button class="toggle-item ${tempSettings.isOfficialCall ? 'selected' : ''}" onclick="updateTempSetting('isOfficialCall', true)">公式戦</button>
        </div>
      </div>
