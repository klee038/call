...
function generatePDF(state) {
  ...
  const getGameRowsHTML = (gameIndex, gameLabelStr) => {
    ...
    if (typeof Recorder !== 'undefined' && state.recorderData) {
        ...
        tableData = Array.from({length: 4}, () => new Array(60).fill(""));
    }
  ...
  let allGamesHTML = `
    <div class="game-section">
      <table class="game-table">
        <colgroup>
          <col style="width: 2.5%;"> <!-- Game Label -->
          <col style="width: 14%;">  <!-- Player Name -->
          <col style="width: 2.5%;"> <!-- S/R -->
          ${Array(29).fill('<col style="width: 2.793%;">').join('')} <!-- 29ラリーマス -->
        </colgroup>
        <tbody>
          ${getGameRowsHTML(0, "1")}
          ${getGameRowsHTML(1, "2")}
